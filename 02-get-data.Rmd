...
AVOCADO - whats the difference between rvest read_html and XML2 read_html
...
```
To save our raw data for case study 1 using the `here` package we can simply type:
```{r}
library(here)
save(coverage, spending, file = here::here("data", "raw_data", "case_study_1.rda"))
#the coverage object and the spending object will get saved as case_study_1.rda within the raw_data directory which is a subdirectory of data. The here package identifies where the project directory is located based on the .Rproj, and thus the path to this directory is not needed.
```
